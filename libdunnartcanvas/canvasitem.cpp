        ...
        const QString& dunnartURI, int pass)
{
    ...
    if (pass == PASS_SHAPES)
    {
                ...
                ! oldConnTypes.contains(type))
        {
            ...
            else if (type == x_shImage)
            {
                QString devIp = nodeAttribute(node, dunnartURI, "ip");
                QString devName = nodeAttribute(node, dunnartURI, "devName");
                QString devMac = nodeAttribute(node, dunnartURI, "devMac");
                QString devFac = nodeAttribute(node, dunnartURI, "devFac");
                QString devOid = nodeAttribute(node, dunnartURI, "devOid");
                QString devType = nodeAttribute(node, dunnartURI, "devType");
                QString devMaskList = nodeAttribute(node, dunnartURI, "devMask");
                //QString devSubnetList = nodeAttribute(node, dunnartURI, "devSubnet");

                IDBody devInfo;
                QStringList strListMask = devMaskList.split(",", QString::SkipEmptyParts);
                foreach(QString strMask, strListMask)
                {
                    devInfo.msks.push_back(q2s(strMask));
                }
//                QStringList strListSubnet = devSubnetList.split(",", QString::SkipEmptyParts);
//                foreach(QString strSubnet, strListSubnet)
//                {
//                    devInfo.msks.push_back(q2s(strSubnet));
//                }

                devInfo.sysName = q2s(devName);
                devInfo.baseMac = q2s(devMac);
                devInfo.devFactory = q2s(devFac);
                devInfo.sysOid = q2s(devOid);
                devInfo.devType = q2s(devType);

                shape = new Image(q2s(devIp), devInfo);
            }
    ...
    else if (pass == PASS_CONNECTORS)
    {
        if (type == x_connector)
        {
            ...
            QString strLeftNodeType = nodeAttribute(node, dunnartURI, "LeftNodeType");
            QString strIpLeft = nodeAttribute(node, dunnartURI, "IpLeft");
            QString strInfLeft = nodeAttribute(node, dunnartURI, "InfLeft");
            QString strIpRight = nodeAttribute(node, dunnartURI, "IpRight");
            EDGE edge;
            edge.ip_left = q2s(strIpLeft);
            edge.inf_left = q2s(strInfLeft);
            edge.ip_right = q2s(strIpRight);
            conn->setEdgeInfo(edge);
            conn->setLeftNodeType(strLeftNodeType);
//            if (strLeftNodeType == "1" || strLeftNodeType == "2" || strLeftNodeType == "3" || strLeftNodeType == "0")
//            {
//                monitor *devMonitor = new monitor(strIpLeft, true, q2s(strInfLeft));
//                devMonitor->Start();
        ...
        QDomDocument& doc)
{
    ...
    if (subset & XMLSS_IOTHER)
    {
        ...
        if (itemType() == "shImage")
        {
            newProp(node, "ip", m_ip.c_str());

            newProp(node, "devMac", m_devInfo.baseMac.c_str());

            newProp(node, "devType", m_devInfo.devType.c_str());

            newProp(node, "devFac", m_devInfo.devFactory.c_str());

            newProp(node, "devOid", m_devInfo.sysOid.c_str());

            newProp(node, "devName", m_devInfo.sysName.c_str());

            QString strDevMaskList;
            std::vector<std::string>::iterator it = m_devInfo.msks.begin();
            for(it; it != m_devInfo.msks.end(); it++)
            {
                strDevMaskList += (*it).c_str();
                strDevMaskList += ",";
            }
            newProp(node, "devMask", strDevMaskList);

            QString strDevSubnetList;
            std::vector<std::string>::iterator itSubnet = m_devInfo.ips.begin();
            for(itSubnet; itSubnet != m_devInfo.ips.end(); itSubnet++)
            {
                strDevSubnetList += (*itSubnet).c_str();
                strDevSubnetList += ",";
            }
            newProp(node, "devSubnet", strDevSubnetList);
        }
...
QString CanvasItem::getIp(void) const
{
    return m_ip.c_str();
}
list<IFREC> CanvasItem::getIFRECList(void) const
{
    return m_devIfList;
}
}
