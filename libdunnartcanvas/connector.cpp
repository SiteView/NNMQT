      ...
      devMonitor(NULL)
{
    ...
    // dark sea green
    m_iLineColor = Qt::darkGreen;
//    QColor col(143, 188, 143);
    setColour(m_iLineColor);

    m_iThresholdSpeed = 1;
    m_iFlowDialogUnitIndex = 0;
}
        ...
        QDomDocument& doc)
{
    ...
    if (subset & XMLSS_ICONNS)
    {
        ...
        newProp(node, "LeftNodeType", m_strLeftNodeType);
        newProp(node, "IpLeft", m_edge.ip_left.c_str());
        newProp(node, "InfLeft", m_edge.inf_left.c_str());
        newProp(node,"IpRight", m_edge.ip_right.c_str());
    }
...
void Connector::RecivePortFlow(int iSpeed)
{
    int iTempThresholdSpeed = m_iThresholdSpeed;
    if (m_iFlowDialogUnitIndex == 0)
    {
        iTempThresholdSpeed = 1024 * m_iThresholdSpeed;
    }

    //qDebug() << "iSpeed : " << iSpeed << " iThreshold : " << m_iThresholdSpeed;
    if (iSpeed > iTempThresholdSpeed)
    {
        m_iLineColor = Qt::red;
    }
    else
    ...
    else
    {
        QString strSpeed = QString::number(iSpeed);
        strSpeed += "Kbps";
        //m_iLineColor = QColor(143, 188, 143);
        setLabel(strSpeed);
    }
}

void Connector::setFlowThreshold(int iSpeed, int iIndex)
{
    m_iThresholdSpeed = iSpeed;
    m_iFlowDialogUnitIndex = iIndex;
    emit modifyFlow(m_iThresholdSpeed, m_iFlowDialogUnitIndex);
}

void Connector::flowThreChanged(int iSpeed, int iIndex)
{
    m_iThresholdSpeed = iSpeed;
    m_iFlowDialogUnitIndex = iIndex;
}

void Connector::setLeftNodeType(QString strType)
{
    m_strLeftNodeType = strType;

    if (strType == "1" || strType == "2" || strType == "3" || strType == "0")
    {
        devMonitor = new monitor(m_edge.ip_left.c_str(), true, m_edge.inf_left);
        devMonitor->Start();
        connect(devMonitor, SIGNAL(SendPortFlow(int)), this, SLOT(RecivePortFlow(int)));
        connect(this, SIGNAL(stopMonitorThread()), devMonitor, SLOT(stopMonitor()));
        //monitorList.push_back(devMonitor);
    }
}

void Connector::setEdgeInfo(EDGE edge)
{
    m_edge = edge;
}

void Connector::deleteMonitor()
{
    if (NULL != devMonitor)
    {
        emit stopMonitorThread();
    }
}

EDGE Connector::getPortInfo()
{
    return m_edge;
}
        ...
        const QStyleOptionGraphicsItem *option, QWidget *widget)
{
    ...
    painter->setPen(m_iLineColor);
    painter->setFont(canvas()->canvasFont());
    painter->setRenderHint(QPainter::TextAntialiasing, true);

    qreal x = painterPath().pointAtPercent(0.5).x() - painterPath().pointAtPercent(0).x();
    qreal y = painterPath().pointAtPercent(0.5).y() - painterPath().pointAtPercent(0).y();
    qreal hypotenuse = sqrt(pow(x, 2)+pow(y, 2));
    qDebug()<<"x:"<<x<<"y:"<<y;
    qreal cos = x/hypotenuse;
    qreal radian = acos(cos);
    qreal angle = 180/(M_PI/radian);
    if (y<0)
    {
        angle = -angle;
    } else if ((y == 0) && (x<0))
    ...
    qDebug()<<"angle:"<<angle;
    QPointF midpoint = painterPath().pointAtPercent(0.25);
    painter->translate(midpoint);
    painter->rotate(angle);
    painter->translate(-midpoint);
    painter->drawText(midpoint, m_label);
        ...
        QMenu& menu)
{
    ...
    QAction *infInfo;
    if (m_strLeftNodeType == "0" || m_strLeftNodeType == "1" || m_strLeftNodeType == "2" || m_strLeftNodeType == "3")
    {
        infInfo = menu.addAction(
                tr("Flow Threashold Setting"));
    }
    ...
    if (action == infInfo)
    {
        ...
        if (NULL == m_pInterfaceInfo)
        {
            m_pInterfaceInfo = new InterfaceInfo();
            //QString strWindowTitle = "Source ip : " + m_edge.ip_left.c_str() + " Dest ip : " + m_edge.ip_right.c_str();
            QString strWindowTitle;
            strWindowTitle += "Source ip : ";
            strWindowTitle += m_edge.ip_left.c_str();
            strWindowTitle += " Dest ip : ";
            strWindowTitle += m_edge.ip_right.c_str();;

            m_pInterfaceInfo->setWindowTitle(strWindowTitle);
            //connect(m_pInterfaceInfo, SIGNAL(flowThreChanged(int)), this, SLOT(flowThreChanged(int)));
            connect(m_pInterfaceInfo, SIGNAL(flowThreChanged(int,int)), this, SLOT(flowThreChanged(int,int)));
            connect(this, SIGNAL(modifyFlow(int,int)), m_pInterfaceInfo, SLOT(onModifyFlow(int,int)));
        }

        m_pInterfaceInfo->show();
    }
