...
typedef struct{
        ...
        std::string devTypeName; //设备类型名称
        std::string devFac;      //设备厂家
        std::string devModel;    //设备型号
}DEVICE_PRO;
...
//Q_DECLARE_METATYPE(std::list<IFREC>)

//附加topo数据条目
typedef struct
{
        std::string localPortInx;//本端端口索引
        std::string localPortDsc;//本端端口描述
        std::string PeerID;      //对端设备ID
        std::string PeerIP;      //对端设备IP
        std::string PeerPortInx; //对端端口索引
        std::string PeerPortDsc; //对端端口描述
}DIRECTITEM;
...
//VRRP
typedef struct{
        std::vector<VRID> vrids;
        std::vector<std::string> assoips;
}VRRPITEM;

//by zhangyan 2009-01-09 (for vrrp,hsrp)
typedef struct
{
        std::vector<std::string> virtualIps;
        std::vector<std::string> virtualMacs;
}RouterStandbyItem;

typedef std::map<std::string, RouterStandbyItem> RouterStandby_LIST; //vrrp,hsrp等
...
//设备stp列表
typedef std::map<std::string,std::list<std::string> > STP_LIST; //add by jiangshanwen 20100910
//边列表
...
typedef std::list<std::list<std::string> > ROUTEPATH_LIST;
...
typedef struct
{
	...
	std::string snmp_version;	// 指定特定SNMP Version的设备
        std::list<std::pair<std::string, std::string> > SNMPV_list; // add by zhangyan 2009-01-04
...
typedef bool (*callback_functor_type)(std::string, std::string);

//特殊设备的oid集合 [{厂商oid,[{指标代码,指标oid}]}] add by jiangshanwen 2010-7-21 
typedef std::map<std::string, std::map<std::string, std::string> > SPECIAL_OID_LIST;


const std::string ROUTE_SWITCH = "0";
const std::string SWITCH = "1";
const std::string ROUTER = "2";
const std::string FIREWALL = "3";
const std::string SERVER = "4";
const std::string PC = "5";
const std::string OTHER = "6";


const std::string ERR_AFT_LOG = "Err_Aft_log.txt";
const std::string ERR_ARP_LOG = "Err_Arp_log.txt";
const std::string ERR_OID_LOG = "Err_Oid_log.txt";
const std::string ERR_INF_LOG = "Err_Inf_log.txt";
const std::string ERR_NBR_LOG = "Err_Nbr_log.txt";
const std::string ERR_RT_LOG  = "Err_Rt_log.txt";
const std::string ERR_BGP_LOG = "Err_Bgp_log.txt";
const std::string ERR_VRRP_LOG = "Err_Vrrp_log.txt";
const std::string ERR_DRC_LOG = "Err_DRC_log.txt";
